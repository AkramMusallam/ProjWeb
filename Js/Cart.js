        ...
        div.innerHTML = `
            <div class="cart-left">
                ...
                <div>
                    ...
                    <p class="item-price">$${item.price.toLocaleString()} ${qty > 1 ? '× ' + qty + ' = $' + (item.price * qty).toLocaleString() : ''}</p>
...
function clearCart() {
    if (getCart().length === 0) { showNotification('Cart is already empty', 'info'); return; }
...
function checkout() {
    ...
    if (confirm(`Order Total: $${total.toLocaleString()}\n${cart.length} items\n\nConfirm purchase?`)) {
        ...
        localStorage.removeItem('cart');
        updateCartBadge();
        showNotification(`Order placed successfully! Thank you, ${user}`, 'success');
        setTimeout(() => loadCart(), 1500);
    }
}

function checkReturnToCheckout() {
    if (localStorage.getItem('returnToCheckout') === 'true' && localStorage.getItem('loggedInUser')) {
        localStorage.removeItem('returnToCheckout');
        setTimeout(() => {
            if (confirm('Continue with checkout?')) checkout();
        }, 500);
    }
}
